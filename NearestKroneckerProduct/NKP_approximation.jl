...
for ci in CartesianIndices(Fld)
  ...
  Fld[ci] = cos(2*π*x)*sin(2*π*y) + 0.5*rand(rng)
end
...
pcm1   = axs[1].pcolormesh(Xf2D,Yf2D,Fldf,vmin=Fmin,vmax=Fmax)
pcm1.set_cmap(cm2)
cb1    = colorbar(pcm1,location="left")

pcm2   = axs[2].pcolormesh(Xf2D,Yf2D,Fldf_r,vmin=Fmin,vmax=Fmax)
pcm2.set_cmap(cm2)
#cb2    = colorbar(pcm2,location="top")

pcm3   = axs[3].pcolormesh(Xf2D,Yf2D,(Fldf .- Fldf_r),vmin=Fmin,vmax=Fmax)
pcm3.set_cmap(cm2)
