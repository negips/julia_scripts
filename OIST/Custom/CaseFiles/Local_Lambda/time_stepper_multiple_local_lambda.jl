...
for i in 1:nsteps
  ...
  if verbosestep>0 && mod(i,verbosestep)==0
    println("Step: $i/$nsteps, Time: $t, Atot/A_eq = $(A_tot/A_eq), γ = $(θpar)")
  end
...
#         PlotContainers[7][1].remove()
    ...
    if ifdynplot
      ...
      λpl = ax4.plot(fld[:,2]*Anorm,fld[:,3],color=cm(4-1),linestyle="-");
    end

    # Dynamic null-clines
    if ifdynnull
      PlotContainers[5] = ax1.plot(ft(λpar),yin,linestyle="--",linewidth=2,color=cm(0));
      PlotContainers[6] = ax1.plot(gt(θpar*λnorm),yin,linestyle="--",linewidth=2,color=cm(1));
    end  
