...
println("F(x,y) Translation along Slope: $ϕfd Degrees")
ϕf      = ϕfd*π/180.0
...
for λ in θvalues
  ...
  local g2(x,y)       = RotLinearFXY5(x,y,θ,X00,pars.gc0,pars.gcx,pars.gcy)
...
ff(x,y,z) = TransFXY(x,y,z,ϕf,pars.fc0,pars.fcx,pars.fcy)
ft(z)     = GetDynamicNullCline(ff,yin,z)
gg(x,y,z) = RotLinearFXY5(x,y,z,X00,pars.gc0,pars.gcx,pars.gcy)
...
Anorm             = 0.1
λnorm             = dθ*π/180.0
F(x,y,z)          = TransFXY(x,y,0.0,ϕf,pars.fc0,pars.fcx,pars.fcy)/ϵ
G(x,y,z)          = RotLinearFXY5(x,y,z,X00,pars.gc0,pars.gcx,pars.gcy)
H(x,y,z)          = FXY(y,z,parsS.fc0,parsS.fcx,parsS.fcy)./δ

Flow(x,y,z)       = [G(x,y,z.*λnorm) F(x,y,z) H(x,y.*Anorm,z)]
...
if xin !="x"
  # close(h4)
  include("time_stepper_multiple_local_lambda.jl")
end
