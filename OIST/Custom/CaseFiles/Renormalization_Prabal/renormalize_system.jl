...
# end 
# #---------------------------------------------------------------------- 
function renormalize_system!(pars,ifalpha)
  ...
  if (ifalpha)
    ...
    pars.fc0      = pars.fc0/α
    for j in 1:length(pars.fcx)
      pars.fcx[j] = pars.fcx[j]/α
    end

    for j in 1:length(pars.fcy)
      pars.fcy[j] = pars.fcy[j]/α
    end

    pars.gc0      = pars.gc0/α
    for j in 1:length(pars.gcx)
      pars.gcx[j] = pars.gcx[j]/α
    end

    for j in 1:length(pars.gcy)
      pars.gcy[j] = pars.gcy[j]/α
    end
  end  

  return Anorm,Bnorm,α
end 
#---------------------------------------------------------------------- 
...
#---------------------------------------------------------------------- 
function renormalize_λsystem!(parsS,ifgamma)
  ...
  if (ifgamma)
    ...
    parsS.fc0      = parsS.fc0/γ
    for j in 1:length(parsS.fcx)
      parsS.fcx[j] = parsS.fcx[j]/γ
    end

    for j in 1:length(parsS.fcy)
      parsS.fcy[j] = parsS.fcy[j]/γ
    end

    parsS.gc0      = parsS.gc0/γ
    for j in 1:length(parsS.gcx)
      parsS.gcx[j] = parsS.gcx[j]/γ
    end

    for j in 1:length(parsS.gcy)
      parsS.gcy[j] = parsS.gcy[j]/γ
    end
  end  
