...
# F,G
set               = sets[1]   # Symmetric
pars              = GetNullClineParams(set) 
set2              = sets[2]   # Asymmetric
pars2             = GetNullClineParams(set2) 
# λ
set               = 56
...
if (ifrenorm) 
  ...
  Anorm2,Bnorm2   = renormalize_system!(pars2)
...
for λ in θvalues
  ...
  local g3(x,y)       = RotXYFXY(x,y,Axis_X0,Axis_Y0,θ,pars2.gc0,pars2.gcx,pars2.gcy)
  ...
  local dτ            = 1.0e-3
  local nsteps        = 200000
  local g30x,g30y     = NullClines(g3,xi,yr0,yr1,nsteps,dτ)
  global plc         += 1
  PlotContainers[plc] = ax1.plot(g30x,g30y,linestyle="--",label="θ=$λ")
