function AssembleMatrixModDiff(U,γ,c0,cnv,lap,xm1,bm1,Basis,lx1,nel,prec)
...
function AssembleMatrixModDiffSparse(U,γ,c0,cx0,whichsrc,cnv,lap,xm1,bm1,Basis,lx1,nel,prec)
...
#---------------------------------------------------------------------- 
function AssembleAdjointModDiffSparse(U,γ,c0,cx0,whichsrc,cnv,lap,xm1,bm1,Basis,lx1,nel,prec)
...
#       Sign for convection term changes in Adjoint        
        Conv[j1:j2,j1:j2] = U.*cnv[:,:,i]
        Src[j1:j2,j1:j2]  = Mμ
        Lap[j1:j2,j1:j2]  = γ2.*lap[:,:,i]
        if i==nel
          Lap[j2,j2]      = Lap[j2,j2] - U/γ2
        end 

#       Sub matrix 
        subm = U.*cnv[:,:,i] + Mμ + γ2.*lap[:,:,i];
        OP[:,:,i] = subm
        A[j1:j2,j1:j2] = A[j1:j2,j1:j2] + subm;
