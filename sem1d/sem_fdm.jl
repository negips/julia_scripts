...
bw12   = PolynomialBases.barycentric_weights(z12);
jgl12  = interpolation_matrix(Geomx.xm1[:,1],z12,bw12);

dgl12  = zeros(prec,lx1,lx1)
...
for i in 1:lx1
  ...
  for j in 1:lx1
    v = zeros(prec,lx1)
    v[j] = 1.0
    dgl12[i,j] = derivative_at(zi,v,z12,bw12)
  end
end  

bm12  = Diagonal(Geomx.bm1[:,1])
...
C0    = 1.0
f     = -(C0)*ones(prec,lx1,1);
fn    = -(C0)*ones(prec,lx1,1);
f12   = -(C0)*ones(prec,lx1,1);
soln  = (C0/2.0)*(xf.^2 .- 1.0)
...
unek        = inv(lapnek)*massnek*fn
unew        = inv(lapNew)*massNew*f12
