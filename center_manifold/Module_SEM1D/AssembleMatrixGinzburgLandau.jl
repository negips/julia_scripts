...
function AssembleMatrixGLSparse(δ::Vector{T},Inp::SEM_Input,GeoM::SEM_GeoMat{GT},B::NodalBasis) where {T<:Number,GT<:Number}
...
#     A     - Combined Linear operator:    Lψ = (δ[1]d/dx + δ[2]I + δ[3]x + δ[4]d²/dx²)ψ
      ...
      # Fd   = spzeros(Prec,dof,dof)
      Binv = zeros(Prec,lx1,nel);
...
#       Sub matrix 
        subm = δ[1].*GeoM.Convd[:,:,i] + Mμ + δ[4]*GeoM.WLap[:,:,i];
        OP[:,:,i] = subm
