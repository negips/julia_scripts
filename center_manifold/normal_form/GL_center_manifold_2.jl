...
δμx_min     = minimum(abs.(δμx_v))
δμx_max     = maximum(abs.(δμx_v))

SL_δμx_v    = LinRange(δμx_min,δμx_max,nω)
SL_Ω_v1     = zeros(Float64,nω)
SL_Ω_v1[1]  = 1.0

for jj in 2:length(SL_δμx_v)
...
#rcParams["markers.fillstyle"] = "none"
ax4.plot(SL_δμx_v/abs(μx),SL_Ω_v1,linestyle="-",linewidth=2,label="Center-Manifold")
#rcParams["markers.fillstyle"] = "full"
...
SL_δμx_v2   = LinRange(δμx_min,δμx_max,nω)
SL_Ω_v12    = zeros(Float64,nω)

for jj in 1:length(SL_δμx_v)
...
#ax5 = gca() 
ax4.plot(-δμx_v2/abs(μx),Ω_v12,linestyle="none",marker="o",markersize=8,label="Ginzburg-Landau - 2")
#rcParams["markers.fillstyle"] = "none"
ax4.plot(SL_δμx_v2/abs(μx),SL_Ω_v12,linestyle="-",linewidth=2,label="Center-Manifold - 2")
#rcParams["markers.fillstyle"] = "full"
...
δγ_min      =  minimum(abs.(δγ_v))
δγ_max      =  maximum(abs.(δγ_v))

SL_δγ_v     = -LinRange(δγ_min,δγ_max,nω)
SL_Ω_v2     =  zeros(Float64,nω)
SL_Ω_v2[1]  =  1.0

for jj in 2:length(SL_δγ_v)
...
#rcParams["markers.fillstyle"] = "none"
ax6.plot(-SL_δγ_v/abs(γ),SL_Ω_v2,linestyle="-",linewidth=2,label="Center-Manifold")
#rcParams["markers.fillstyle"] = "full"
...
SL_δγ_v2    = -LinRange(δγ_min,δγ_max,nω)
SL_Ω_v22    =  zeros(Float64,nω)

for jj in 1:length(SL_δγ_v2)
...
#ax7 = gca() 
ax6.plot(-δγ_v2/abs(γ),Ω_v22,linestyle="none",marker="o",markersize=8,label="Ginzburg-Landau - 2")
#rcParams["markers.fillstyle"] = "none"
ax6.plot(-SL_δγ_v2/abs(γ),SL_Ω_v22,linestyle="-",linewidth=2,label="Center-Manifold - 2")
#rcParams["markers.fillstyle"] = "full"
