...
# BaseState   = load("GL_BaseState3.jld2")
# δ5          = BaseState["δ5"]
# U           = BaseState["U"]
# γ           = BaseState["γ"]
# μx          = BaseState["μx"]
...
#BaseState   = load("GL_BaseState2.jld2")
#basestate   = BaseState["basestate"]
#δ5          = BaseState["δ5"]
#A3          = δ5*3.0*spdiagm(basestate.*basestate)
#OPg        .= OPg .+ A3 
...
if (ifsave)
  if (ifadjoint)
    fname = "adjoint_GL_5.jld2"
  else
    fname = "direct_GL_5.jld2"
  end

  save(fname,"evs",λ,"evec",eigvec,"xg",xg,"Bg",Bg);
  println(fname*" saved.")
end 
