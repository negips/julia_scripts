...
# Harmonic Modes
nHar        = 1                           # No. of Harmonic modes
ndofE       = ndof + nHar
# Harmonic angular frequencies
λH          = zeros(ComplexF64,nHar)
for i in 1:nHar
  λH[i]       = 2.0*im
end
...
# Krylov-Schur/Arnoldi subspace allocations
...
BgE         = zeros(vt,ndofE)
BgE[1:ndof] = copy(Bg)
# Mass for Harmonic parts
for i in ndof+1:ndofE
  BgE[i]    = 1.0
end
...
# Build Forcing
Harf        = spzeros(ComplexF64,ndofE,nHar)
σ           = zeros(Float64,nHar)         # Std. Deviation of the Gaussian
xfm         = zeros(Float64,nHar)         # Mean location of the Gaussian
for i in 1:nHar
  xfm[i]      = 10.0
  σ[i]        = 1.0
end
for i in 1:nHar
  Harf[1:ndof,i] = 1.0*exp.(-((xg .- xfm[i])/σ[i]).^2)
  Harf[ndof+i,i] = λH[i]
end
...
AOPgE       = spzeros(vt,ndofE,ndofE)

while (~ifconv)
  global V,H,v
  global t, i
  global plr,pli
  global OPg, AOPg, OPgE, AOPgE
  global nkryl
  global hλ, ax1
  global ifconv
  global major_it
...
# Build the operator only the first time
  if (i==1)
    ...
    AOPgE[1:ndof,1:ndof]         = copy(AOPg)
    AOPgE[ndof+1:ndofE,:]        = Harf'
    # Boundary Conditions
    AOPgE[1,:]                   = zeros(VT,1,ndofE)
    AOPgE[1,1]                   = one + im*zro        # Change operator for BC

    # Direct Operator   
    OPgE[1:ndof,1:ndof]          = copy(OPg)
    OPgE[1:ndofE,ndof+1:ndofE]   = copy(Harf)
    # Boundary Conditions
    OPgE[1,:]                    = zeros(VT,1,ndofE)
    OPgE[1,1]                    = one + im*zro        # Change operator for BC
