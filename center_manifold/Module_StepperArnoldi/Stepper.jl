...
#---------------------------------------------------------------------- 
function REStepArn(L::AbstractMatrix{T1},B::AbstractVector{T2},V0::AbstractMatrix{T1},W0::AbstractMatrix{T1},restriction::Vector{Bool},f::AbstractVector{T1},ω::T3,StpInp::StepperInput,ArnInp::ArnoldiInput,lbc::Bool,rbc::Bool) where {T1,T2,T3<:Number}

  dtype           = T1

  N               = size(L,2)
  Ne              = N + 1
  n               = size(V0,2)      # Restriction Subspace size
  ...
  while (~ifconv)
    ...
    for i in 1:nsteps
      ...
      if (resonance)
        RE_BRK4!(ve,L,Be,V0,W0,restriction,f,ω,lbc,rbc,ve1,ve2,ve3,ve4,ve5,dt)
      else        
        E_BRK4!(ve,L,Be,f,ω,lbc,rbc,ve1,ve2,ve3,ve4,dt)
      end
    end  
  
    # Expand Krylov space
    Ve,H,nkryl,β,major_it = IRAM!(Ve,H,Be,ve,nkryl,lkryl,major_it,nev,Ω,ngs,verbose)
    ve                    = Ve[:,nkryl]
